...
def _flatten_list(obj):
    ...
    for item in obj:
        if isinstance(item, (list, tuple, set)):
            gather.extend(_flatten_list(item))            
        else:
            gather.append(item)
    return gather
...
def simplify_graph(G, strict=True, remove_rings=True, track_merged=False):
    ...
    # define edge segment attributes to combine in a single list
    attrs_to_merge = {"osmid", "u_original", "v_original", "merged_edges"}
    ...
    for path in _get_paths_to_simplify(G, strict=strict):
        ...
        for u, v in zip(path[:-1], path[1:]):
            ...
            for attr in edge_data:
                if track_merged and attr == "merged_edges":
                    merged_edges.extend(edge_data[attr])
        ...
        for attr in path_attributes:
            attr_value = path_attributes[attr]
            
            # Special treatment for merged_edges... a bit of a hack !
            if attr == "merged_edges":
                path_attributes[attr] = [item for sublist in attr_value for item in sublist]
                continue
            
            if isinstance(attr_value, (list, tuple, set)):
                attr_value = _flatten_list(attr_value)
            
            if attr in attrs_to_sum:
                # if this attribute must be summed, sum it now
                path_attributes[attr] = sum(attr_value)
            elif len(set(attr_value)) == 1:
                # if there's only 1 unique value in this attribute list,
                # consolidate it to the single value (the zero-th):
                path_attributes[attr] = attr_value[0]
            else:
                # otherwise, if there are multiple values, keep one of each
                path_attributes[attr] = list(set(attr_value))
        
        # Try to construct the new edge geometry by merging the geometries of the edge segments
        # If that's not working, revert to making a new edge based on the nodes themselves.
        try:
            mls = MultiLineString(geometries)
            welded_ls = linemerge(mls)
            path_attributes["geometry"] = welded_ls
        except Exception:
        ...
        if track_merged:
            # Remove duplicates in merged_edges
